...
**Hosting.** The docs are published to GitHub Pages, not ReadTheDocs — we use the RTD theme without the RTD service. `.github/workflows/docs.yml` builds and deploys on every `v*` release tag (the ones `bump-my-version` produces) via GitHub's Pages-from-Actions flow (no `gh-pages` branch). This is deliberately release-gated: between releases, the published site reflects the last shipped version, not unreleased master state. On PRs, `ci.yml`'s `docs-build` job runs the same `sphinx-build -W` but without deploying, so doc regressions fail the PR check before merge. Need to redeploy without cutting a release? Use the **Run workflow** button on the Docs workflow in the Actions UI — `workflow_dispatch` is enabled. One-time repo setup: **Settings → Pages → Build and deployment → Source: GitHub Actions**. Without that, the deploy workflow has nothing to push to.
...
- `docs.yml` — triggers on `v*` release tags (and manual dispatch). Builds the Sphinx site and deploys it to GitHub Pages via `actions/deploy-pages`. Uses a `pages` concurrency group so simultaneous pushes don't race each other into a stuck Pages state. Release-gated by design — so that what's live at https://sixty-north.github.io/asyoulikeit/ reflects the last shipped release, not arbitrary master commits.

To release: run `bump-my-version bump <level>` on a clean `master`, then `git push --follow-tags`. The push of the commit triggers `ci.yml`; the push of the tag triggers both `publish.yml` (to PyPI) and `docs.yml` (to GitHub Pages).
